...
#include "testcase_005.h"

TestCase_005::TestCase_005()
{
...
int TestCase_005::check(int year)
{
    ...
    // The settlers should populate the landmass with several cities.
    if (cities.size() >= 3)
    {
        isdone = true;
        haspassed = true;
    ...
    if (ticks > 20000)
    {
        isdone = true;
        haspassed = false;
        message = "The world was not populated with three cities in time.";
    }

    return 0;
}
std::string TestCase_005::title()
{
    return std::string("Cities build settlers and settlers populate the continent.");

}

bool TestCase_005::done()
{
    return isdone;
}
bool TestCase_005::passed()
{
    return haspassed;
}
std::string TestCase_005::failedMessage()
{
    return message;
}
